...
serve(async (req) => {
  ...
  try {
    ...
    const settings: BotSettings = {
      business_description: botSettings?.business_description || `${workshop.name} es un taller mecánico automotriz.`,
    ...
    const conversationHistory = (messages || [])
      .reverse()
      .map(m => `${m.direction === 'inbound' ? 'Cliente' : 'Taller'}: ${m.text}`)
      .join('\n');

    // Build booking URL - use configured URL directly if it starts with http, otherwise construct it
    let fullBookingUrl: string | null = null;
    if (workshop.booking_url) {
      if (workshop.booking_url.startsWith('http')) {
        fullBookingUrl = workshop.booking_url;
      } else {
        fullBookingUrl = `https://autonomiasuite.lovable.app${workshop.booking_url}`;
      }
    } else if (workshop.slug) {
      fullBookingUrl = `https://autonomiasuite.lovable.app/agenda/${workshop.slug}`;
    }
...
IMPORTANTE: Responde SOLO con JSON válido, sin texto adicional.`;
      ...
      systemPrompt = `Eres el asistente virtual profesional de ${workshop.name}, un taller mecánico automotriz.
