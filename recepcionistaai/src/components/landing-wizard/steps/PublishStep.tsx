...
export function PublishStep({ config, services, team, onPublish, isPublishing = false }: PublishStepProps) {
  ...
  const slug = config?.workshop_id ? config.workshop_id.slice(0, 8) : 'preview';
  const primaryColor = config?.primary_color || '#3b82f6';
  const landingUrl = `${window.location.origin}/agenda/${slug}`;
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      {isPublished && (
        <div className="flex gap-4">
          ...
          <Button className="flex-1" onClick={openLanding}>
