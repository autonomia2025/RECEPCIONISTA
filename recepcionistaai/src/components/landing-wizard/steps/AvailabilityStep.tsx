...
export function AvailabilityStep({ config, onUpdate }: AvailabilityStepProps) {
  ...
  const [lunchStart, setLunchStart] = useState(config?.lunch_break_start || '');
  const [lunchEnd, setLunchEnd] = useState(config?.lunch_break_end || '');
  ...
  useEffect(() => {
    if (config) {
      ...
      setLunchStart(config.lunch_break_start || '');
      setLunchEnd(config.lunch_break_end || '');
  ...
  const handleLunchChange = (field: 'start' | 'end', value: string) => {
    if (field === 'start') {
      setLunchStart(value);
      onUpdate({ lunch_break_start: value || null });
    } else {
      setLunchEnd(value);
      onUpdate({ lunch_break_end: value || null });
    }
  };
      ...
      <div className="space-y-6">
        ...
        <Card className="p-4 bg-background/80">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-medium">Pausa (opcional)</h3>
          </div>
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Horario de colación o descanso donde no se agendan citas
          </p>
        </Card>
