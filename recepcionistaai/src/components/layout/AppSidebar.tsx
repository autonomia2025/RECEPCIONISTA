...
import { LayoutDashboard, MessageSquare, Users, Calendar, UserCog, Building2, LogOut, ChevronLeft, ChevronRight, BarChart3, Settings, Bot, DollarSign, Globe, Activity, Mail, UserPlus } from 'lucide-react';
...
export const AppSidebar = () => {
  ...
  const showLandingButton = !isSuperAdmin && workshopMode?.booking_mode === 'with_scheduling';
  ...
  const navItems = useMemo(() => {
    ...
    return baseItems;
  }, [isSuperAdmin, workshopMode?.booking_mode]);
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'sidebar-gradient min-h-screen min-h-[100dvh] flex flex-col transition-all duration-300 shadow-xl',
          collapsed ? 'w-16' : 'w-64',
          isMobile && 'fixed inset-y-0 left-0 z-40 w-72 overflow-y-auto',
      ...
      {/* Seat Info */}
      {!isSuperAdmin && !collapsed && seatInfo && <div className="px-4 py-3 border-t border-white/10">
