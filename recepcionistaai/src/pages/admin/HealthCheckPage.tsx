...
function StatusBadge({ status }: { status: 'healthy' | 'warning' | 'critical' }) {
  const config = {
    healthy: { label: 'OK', variant: 'default' as const, className: 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' },
    warning: { label: 'Alerta', variant: 'outline' as const, className: 'bg-amber-500/10 text-amber-600 border-amber-500/20' },
    critical: { label: 'Crítico', variant: 'destructive' as const, className: 'bg-red-500/10 text-red-600 border-red-500/20' },
  };
...
export default function HealthCheckPage() {
  const { data: workshops, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      // Obtener todos los workshops activos
      ...
      // Para cada workshop, obtener stats de salud
